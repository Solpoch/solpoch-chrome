...
export default function SignMessage() {
  ...
  useEffect(() => {
    async function getApproval() {
      ...
      try {
        ...
        if (approval?.type === "APPROVAL_SIGN_MESSAGE") {
          const message = new TextDecoder().decode(new Uint8Array(approval.payload.message));
  ...
  const handleApprove = async () => {
    await sendMessage("APPROVAL_MANAGER_RESOLVE_APPROVAL_SIGN_MESSAGE", {
      id: id!,
      approved: true,
      password,
    });
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide flex flex-col gap-3 pb-6">
          ...
          <div>
            <div className="flex gap-4 items-center">
              ...
              <div>
                <h2 className="text-sm">Approve Sign Message Request</h2>
            ...
            <div className="rounded bg-primary/20 p-4 mt-8 flex gap-2">
              ...
              <h3 className="text-xs">By approving, you authorize <span className="text-primary">{origin}</span> to sign this message</h3>
            </div>
          </div>
